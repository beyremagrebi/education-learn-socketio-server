...
io.on("connection", (socket) => {
  ...
  // Listen for the "comment" event to handle comments
  socket.on("comment", async (commentData, tiquetId) => {
    console.log("Comment received:", commentData);

    try {
      // Send the comment data to the appropriate service endpoint
      await axios.post(`http://localhost:5050/tiquets/comment-tiquet/${tiquetId}`, {
        comment: commentData,
      });
